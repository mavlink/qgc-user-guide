...
### AutoLoad plans
When this setting is turned on when QGC connects to a vehicle it will automatically upload a plan to the vehicle. The plan file must be named "AutoLoad#.plan" where the # is replaced with the vehicle id. The location of the plan file is in the file save path as specified above.

### File Save path
You can now specify a save path which QGC will use as the default location to save files such as Parameters, Telemetry or Mission Plans to.
...
The new Plan Toolbar shows at the top of the Plan View. It shows you information related to the currently selected waypoint as well as statistics for the entire mission.

When you are connected to a vehicle it also show an *Upload* button which you can use to upload the plan to the vehicle.

### Mission Settings
When planning a mission the first item in the mission list on the right of the screen is Mission Settings. This allows you to specify values which apply to the entire mission, or settings you want to control right at the beginning of a mission.
...
##### Waypoint alt
This specifies the default altitude for newly adding mission items. If you update this value while you have a mission loaded it will prompt you to update all the the waypoints to this new altitude.
...
Allows you to specify a camera action to take, control the gimbal and set your camera into photo or video mode.
...
The additional value which can be specified when planning a mission is the vehicle flight speed. By specifying this value, total mission or survey times can be approximated even when not connected to a vehicle.
...
This pretty much does what is says, which is to add a landing pattern for fixed wings to your mission. The first point of the pattern is the loiter point which commands to vehicle to loiter to a specific altitude. Once that altitude is reached, the vehicle will begin the landing sequence and fly down to the specified landing spot.
